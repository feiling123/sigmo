...
func callbackHTML() string {
	return `<!doctype html><html><body><script>try{window.parent.postMessage({type:"sigmo-websheet-callback"},"*")}catch(_){}</script>Carrier flow returned to Sigmo.</body></html>`
}
